import { supabase as sharedSupabase } from "./supabaseData";
...
const prepareRecord = (record = {}) => {
  ...
  nextRecord.createdAt = nextRecord.createdAt ? toIsoString(nextRecord.createdAt) : new Date().toISOString();
  nextRecord.updatedAt = nextRecord.updatedAt ? toIsoString(nextRecord.updatedAt) : nextRecord.createdAt;
...
export async function updateDoc(docRef, updates) {
  ...
  const { error } = await supabase.from(docRef.table).update({ ...payload, updatedAt: serverTimestamp() }).eq("id", docRef.id);
  if (error) throw error;
}
...
export const ordersRepository = {
  async getById(orderId) {
    return getTableClient("orders").getById(orderId);
  },

  async getList(filters = {}, pageSize = 20, lastDoc = null) {
    const clauses = [];
    if (filters.status) clauses.push(where("status", "==", filters.status));
    if (filters.paymentStatus) clauses.push(where("paymentStatus", "==", filters.paymentStatus));
    ...
    return {
      orders: snapshot.docs.slice(0, pageSize).map((entry) => entry.data()),
      lastDoc: snapshot.docs[pageSize] || null,
    };
  },
  ...
  async update(orderId, updates) {
    await updateDoc(doc(null, "orders", orderId), updates);
  },
...
export const customersRepository = {
  ...
  async search(searchTerm) {
    ...
    const nameSnapshot = await getDocs(query(collection(null, "customers"), where("name", "ilike", term), orderBy("createdAt", "desc"), limit(50)));
    const emailSnapshot = await getDocs(query(collection(null, "customers"), where("email", "ilike", term), orderBy("createdAt", "desc"), limit(50)));
